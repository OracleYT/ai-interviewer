...
export async function addImageEvidance(data: {
  ...
  console.log(
    `[IMAGE_EVIDENCE] Starting to process image evidence for interview: ${data.interviewId}`
  );

  try {
    console.log(
      `[IMAGE_EVIDENCE] Request payload: ${JSON.stringify(data, null, 2)}`
    );
    ...
    if (!data.evidence) {
      console.warn(
        `[IMAGE_EVIDENCE] Missing required field: evidence for interview ${data.interviewId}`
      );
      return {
        status: 400,
        success: false,
    ...
    console.log(
      `[IMAGE_EVIDENCE] Validation passed. Updating database for interview ${data.interviewId} with violation type: ${data.evidence.title || "unknown"}`
    );

    // Database operation
    console.log(
      `[IMAGE_EVIDENCE] Executing database update for interview ${data.interviewId}`
    );
    ...
    console.log(
      `[IMAGE_EVIDENCE] Successfully updated evidence for interview ${data.interviewId}`
    );
    console.log(
      `[IMAGE_EVIDENCE] Evidence details: title=${data.evidence.title}, violation=${data.evidence.violation}, timestamp=${new Date().toISOString()}`
    );
